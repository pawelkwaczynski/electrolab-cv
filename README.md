...
`validate_against_manual_table.py` reproduces the numbers when the raw files are placed under `data/` (they are not part of this repository).

## Use

Open `ElectroLab_CV_Analyzer.ipynb` in Colab, run the setup cell, upload one folder of scans per electrode. Locally: `uv run --with matplotlib --with scipy --with pandas --with ipython --with openpyxl python validate_against_manual_table.py`.
